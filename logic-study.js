(function(root,factory){
  ...
  function makeReasoningAssertion(kind){
    const map={
      mp:{stem:'If a person studies every day, then that person improves. Jordan studies every day. Therefore Jordan improves.',answer:'Modus Ponens',why:'This affirms the antecedent and validly concludes the consequent.'},
      mt:{stem:'If the alarm is armed, then the indicator light is on. The indicator light is not on. Therefore the alarm is not armed.',answer:'Modus Tollens',why:'This denies the consequent and validly denies the antecedent.'},
      ac:{stem:'If it is raining, then the sidewalk is wet. The sidewalk is wet. Therefore it is raining.',answer:'Affirming the Consequent',why:'The consequent can be true for some other reason, so it does not prove the antecedent.'},
      da:{stem:'If a person is a surgeon, then that person is a doctor. Taylor is not a surgeon. Therefore Taylor is not a doctor.',answer:'Denying the Antecedent',why:'The antecedent being false does not force the consequent to be false.'},
      um:{stem:'All cats are animals. All dogs are animals. Therefore all dogs are cats.',answer:'Undistributed Middle',why:'The middle term is never distributed, so the premises do not connect dogs to cats.'},
      ilmj:{stem:'All dogs are mammals. No cats are dogs. Therefore no cats are mammals.',answer:'Illicit Major',why:'The major term is distributed in the conclusion but not in the major premise.'},
      ilmn:{stem:'All poets are writers. All poets are dreamers. Therefore all dreamers are writers.',answer:'Illicit Minor',why:'The minor term is distributed in the conclusion but not in the minor premise.'},
      four:{stem:'All banks keep money safe. River banks are banks. Therefore river banks keep money safe.',answer:'Four-Term Fallacy',why:'The word “bank” changes meaning, so the argument actually contains four terms.'},
      noerror:{stem:'All surgeons are doctors. All doctors are professionals. Therefore all surgeons are professionals.',answer:'No error / valid reasoning',why:'The middle term properly links the premises, so the categorical syllogism is valid.'}
    };
    const q=map[kind]||map.noerror;
  ...
  function checked(selector,fallback){const arr=[...document.querySelectorAll(selector+':checked')].map(x=>x.value);return arr.length?arr:fallback;}
  function squareQuestion(){
    const cats=checked('#logicSquareCats input',['identify','relations','assertions','translate']);
    ...
    if(cat==='assertions'){const allowed=checked('#logicAssertionKinds input',ASSERTION_FILTERS.map(x=>x.id));return makeSquareAssertion(rand(allowed),s,p);}
    const t=rand(types);const ordinary={A:`Every member of the ${s} group is among the ${p}.`,E:`Not one of the ${s} is among the ${p}.`,I:`At least one of the ${s} is among the ${p}.`,O:`At least one of the ${s} is not among the ${p}.`}[t];return {cat:'Translation',stem:ordinary,choices:shuffle(types.map(x=>prop(x,s,p))),answer:prop(t,s,p),why:`That statement translates to ${prop(t,s,p)}, the ${t} form.`};
  }
  function errorQuestion(){const allowed=checked('#logicErrorCats input',ERROR_FILTERS.map(x=>x[0]));return makeReasoningAssertion(rand(allowed));}
  function cssEscape(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  function renderQuestion(kind,q){const pre=kind==='square'?'logicSquare':'logicError',state=kind==='square'?sqState:erState;state.current=q;state.selected=null;document.getElementById(pre+'Stem').innerHTML=cssEscape(q.stem).replace(/\n/g,'<br>');document.getElementById(pre+'Feedback').innerHTML='';const box=document.getElementById(pre+'Choices');box.innerHTML='';shuffle(q.choices).forEach(c=>{const b=document.createElement('button');b.className='logicChoice';b.textContent=c;b.onclick=()=>{if(state.current!==q)return;[...box.children].forEach(x=>x.classList.remove('selected'));b.classList.add('selected');state.selected=c;};box.appendChild(b);});}
  ...
  return {init,showLogic,relationshipBetween,truthInference,classifyConditionalForm,getAssertionFilterOptions,makeSquareAssertion,makeReasoningAssertion};
});